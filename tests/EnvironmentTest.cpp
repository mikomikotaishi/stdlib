...
void test_args_subspan_iterates_user_arguments() {
    // The Java/C# view of the command line - everything after the program name -
    // is one subspan away. Under ctest there are no user arguments, so the count
    // is what proves the loop visits exactly args()[1..] and nothing else.
    const Span<const StringView> args = Environment::args();
    require(!args.empty(), "arguments are available");

    usize visited = 0;
    for (const StringView argument : args.subspan(1)) {
        expect(argument.data() == args[visited + 1].data(), "iteration sees args()[1..] in order");
        ++visited;
    }
    expect_eq(visited, args.size() - 1, "every argument after the program name is visited once");
}
...
void test_pid_and_operating_system() {
    ...
    expect(
        Environment::compiler() != Environment::Compiler::UNKNOWN,
        "compiler() identifies a supported compiler"
    );
}
...
int main() {
    return run({
        ...
        {"Environment.args_subspan_iterates_user_arguments", test_args_subspan_iterates_user_arguments},
