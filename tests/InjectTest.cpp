...
void test_shared_pointer_retrieval_of_a_singleton() {
    ...
    SharedPointer<Registry> pointer = injector.get<SharedPointer<Registry>>();
    Registry& reference = injector.get<Registry&>();

    expect(pointer != nullptr, "a constructor binding can be requested as a SharedPointer");
    expect(pointer.get() == &reference, "value and SharedPointer keys share singleton storage");
}
